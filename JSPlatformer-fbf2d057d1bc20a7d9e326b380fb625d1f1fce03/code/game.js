...
var actorChars = {
  ...
  "S": Skull,
  "T": PoisonT,  //Top of Poison pools
  "R": PoisonR,  //Rising poison
  "e": Enemy,
  "B": Boss,
  "F": Fireworks,
  "1": Text1,
...
function Player(pos) {
  this.pos = pos.plus(new Vector(0, -0.6));
...
function PoisonR(pos) {
	this.basePos = this.pos = pos.plus(new Vector(0, 13));
	this.size = new Vector(1, 1.2);
    this.rise = 0.1;
	//this.wobble = Math.random() * Math.PI * 2;
}
PoisonR.prototype.type = "poisonr";

function PoisonT(pos) {
	this.basePos = this.pos = pos.plus(new Vector(0,0));
	this.size = new Vector(1, 1.2);
	this.wobble = Math.random() * Math.PI * 2;
}
PoisonT.prototype.type = "poisont";

function Enemy(pos){
	this.basePos = this.pos = pos.plus(new Vector(0, -2.2));
	this.size = new Vector(3.2, 3.2);
	this.move = Math.random() * Math.PI * 2;
}
Enemy.prototype.type = "enemy";

function Boss(pos){
	this.basePos = this.pos = pos.plus(new Vector(0, -8));
	this.size = new Vector(11.15, 11.15);
	this.run = /*Math.random() * Math.PI */ 2;
}
Boss.prototype.type = "boss";

function Fireworks(pos){
	this.pos = pos;
	this.size = new Vector(6.95, 7.3);
}
Fireworks.prototype.type = "fireworks";

function Text1(pos){
	this.basePos = this.pos = pos.plus(new Vector(0, -2));
	this.size = new Vector(5, 5);
}
Text1.prototype.type = "text1";
...
Skull.prototype.act = function(step) {
...
var riseSpeed = 0.015, riseDist = -100;
PoisonR.prototype.act = function(step){
	this.rise += step * riseSpeed;
	var risePos = Math.sin(this.rise) * riseDist;
	this.pos = this.basePos.plus(new Vector(0, risePos));
};

PoisonT.prototype.act = function(step){
	this.wobble += step * wobbleSpeed;
	var wobblePos = Math.sin(this.wobble) * wobbleDist;
    this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

var moveSpeed = 5, moveDist = 3;
...
var runSpeed = .089, runDist = -56;
Boss.prototype.act = function(step){
	this.run += step * runSpeed;
	var runPos = Math.sin(this.run) * runDist;
	this.pos = this.basePos.plus(new Vector(runPos, 0));
};

Text1.prototype.act = function(step){};
Text2.prototype.act = function(step){};
Text3.prototype.act = function(step){};
Text4.prototype.act = function(step){};
Text5.prototype.act = function(step){};

Fireworks.prototype.act = function(step){};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "poison" || type == "skull" || type == "poisonr" || type == "enemy" || type == "boss" || type == "poisont" && this.status == null) {
	  //this.player.motion = 0;
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
